...
export default {
  ...
  methods: {
    ...
    addApply() {
      ...
      this.$axios.post("/api/api/appInfo", formData).then(response => {
        if (response.status === 200) {
          this.$message({
            type: "success",
    ...
    del(index) {
      ...
      this.$confirm(
        "是否确认删除应用：" + rowData.enName + " " + rowData.chName + "?",
        "提示",
        {
        ...
        this.$axios.get("/api/api/appInfo/" + rowData.appId).then(response => {
          if (response.status === 200) {
            console.log("删除成功");
            this.$message({
              type: "success",
